...
template<typename T>
font<T>::font(unsigned char const *const memory_offset,
...
template<typename T>
double font<T>::get_height() const {
  return height;
}
...
glyph<T> &font<T>::load_glyph_from_freetype(char const thischar, glyph_map_iterator_type &it) {
  ...
  it = glyphs.emplace(thischar, glyph<T>(thischar, static_cast<float>(static_cast<double>(face->glyph->metrics.horiAdvance) * scale))).first;
  glyph<T> &thisglyph(it->second);
  for(int point_start = 0, c = 0; c < outline.n_contours; ++c) {                // can n_contours be contours?  I've no idea
    thisglyph.contours.emplace_back();
    auto &thiscontour(thisglyph.contours.back());
    ...
    for(int p = point_start; p <= point_end; ++p) {                             // note: we're NOT stopping one short of the end point, last segment goes on top
      ...
      case point::types::ON:
        if(thiscontour.segments.back().points.size() == 1) {                    // is this the first point of the segment?
          ...
          case point::types::ON:
            thiscontour.segments.back().type = segment<T>::types::LINE;         // this was a straight line
            #ifdef DEBUG_VECTORFONTSTORM_DETAILED
          ...
          case point::types::OFF_THIRDORDER:
            thiscontour.segments.back().type = segment<T>::types::CUBIC;        // this was a cubic bezier curve
            #ifdef DEBUG_VECTORFONTSTORM_DETAILED
          ...
          case point::types::OFF_SECONDORDER:                                   // this is a continuing conic bezier
            {
              ...
              thiscontour.segments.back().points.emplace_back(thispoint);       // copy this point to start the next segment
              auto &lastsegment(thiscontour.get_second_to_last_segment());      // the one before this one we just pushed
